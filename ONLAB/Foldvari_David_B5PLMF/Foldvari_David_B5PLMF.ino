...
char DaysOfTheWeek[7][10]={
  {'V','a','s',0xE1,'r','n','a','p','\0'},
  {'H',0xE9,'t','f',0xF5,'\0'},
  {'K','e','d','d','\0'},
  {'S','z','e','r','d','a','\0'},
  {'C','s',0xFC,'t',0xF6,'r','t',0xF6,'k','\0'},
  {'P',0xE9,'n','t','e','k','\0'},
  {'S','z','o','m','b','a','t','\0'} 
};
/***************************************************************///LM75
...
uint8_t LastShowedMode = DateMode;
WiFiClient client;
...
void GetDate(void) {
  ...
  TextArray[i++] = '.';
  TextArray[i++] = ',';
  for (uint8_t j = 0; DaysOfTheWeek[dayOfWeek][j]!= '\0'; j++) {
    TextArray[i++] = DaysOfTheWeek[dayOfWeek][j];
  }
  TextArray[i++] = ' ';
  TextArray[i++] = ' ';
  TextArray[i++] = ' ';
...
void CreateTimeDisplayFrame() {
  ...
  if((hour_ten_old != hour_ten_new&&hour_single_old != hour_single_new&&min_ten_old != min_ten_new&&min_single_old != min_single_new)||LastShowedMode==DateMode){
    hour_ten_old = hour_ten_new;
    hour_single_old = hour_single_new;
    min_ten_old = min_ten_new;
    min_single_old = min_single_new;
    TimeChanged=true;
    LastShowedMode=TimeMode;  
  }
  else{
    TimeChanged=false;
  }
...
void ICACHE_RAM_ATTR onTimerISR() {
  ...
  if (adcInterruptCounter == 2) { //1s megszakÄ‚Â­tÄ‚Ë‡s
