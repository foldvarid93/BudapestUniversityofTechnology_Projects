...
/******************************************************************************
 * @brief Display individual segments to the "Text part" 7x14 segment display
 * @param seegmentField segments to display. An array with 7 members. Array
 * member represents a segment in the following way
 *  -------0------
 *
 * |   \5  |6  /7 |
 * |2   \  |  /   |1
...
void displaySegmentField(SegmentLCD_SegmentData_TypeDef* segmentField)
{
	 ...
	 for (s = 0; s < 7; s++)
	 {
		 for (i = 0; i < 14; i++)
	     	 {
			   bit = EFM_Display.Text[s].bit[i];
			   com = EFM_Display.Text[s].com[i];

			 	 if (segmentField[s].raw & (1 << i))
			 	 {
			 		 /* Turn on segment */
			 		 LCD_SegmentSet(com, bit, true);
			 	 }
